...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.d("my-api", "onCreate was called!");
        ...
        JsonArrayRequest jsonArrayRequest = new JsonArrayRequest(Request.Method.GET,apiURL,null,new Response.Listener<JSONArray>(){
            ...
            public void onResponse(JSONArray response) {
                try {
                    ...
                    recordsGrouped = allData.stream().filter((record) -> record.getName() != "null") //filter out all names with null
                            .filter((record) -> record.getName().length() >= 1)                                                 //filter out all the names with no text
                            ...
                            .collect(Collectors.groupingBy(w -> w.getListId()));                                                //group the lists by listIds and set list Id groups to the value of the listId key in the map
